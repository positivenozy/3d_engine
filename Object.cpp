...
// применяется заданная матрица к прикреплённым объектам (к заданному объекту)
...
void Object::rotateUp(double ru) {
    _angleLeftUpLookAt = Vec3D(_angleLeftUpLookAt.x(), _angleLeftUpLookAt.y(), _angleLeftUpLookAt.z() + ru);
    rotate(up(), ru);
}

void Object::rotateLookAt(double rlAt) {
    _angleLeftUpLookAt = Vec3D(_angleLeftUpLookAt.x(), _angleLeftUpLookAt.y() + rlAt, _angleLeftUpLookAt.z());
    rotate(lookAt(),rlAt);
}
